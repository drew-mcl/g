plugins {
    ...
    `kotlin-dsl`
    `maven-publish`
    id("com.company.build.dependency-graph") apply false
}
...
// Apply dependency-graph plugin only when requested via CLI
if (gradle.startParameter.taskNames.contains("generateDependencyGraph")) {
    apply(plugin = "com.company.build.dependency-graph")
}
