...
class AsgardCodeQualityPlugin : Plugin<Project> {
    ...
    private fun configureCheckstyle(project: Project) {
        project.configure<org.gradle.api.plugins.quality.CheckstyleExtension> {
            toolVersion = "10.12.5"
            // Reference from root project's gradle directory
            val configFile = project.rootProject.file("gradle/checkstyle/checkstyle.xml")
            if (configFile.exists()) {
                this.configFile = configFile
            } else {
                // Fallback to local file if root not found
                this.configFile = project.file("gradle/checkstyle/checkstyle.xml")
            }
        }
    ...
    private fun configurePmd(project: Project) {
        project.configure<org.gradle.api.plugins.quality.PmdExtension> {
            toolVersion = "6.55.0"
            // Reference from root project's gradle directory
            val rulesetFile = project.rootProject.file("gradle/pmd/ruleset.xml")
